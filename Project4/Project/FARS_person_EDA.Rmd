...
```{r crashes_by_hour}
person %>%
    group_by(ST_CASE, HOUR) %>%
    filter(HOUR != 99) %>%
    # There is almost certainly a more clever way to do next 3 lines...
    summarise(unique_crashes = 1) %>%
    group_by(HOUR) %>%
    summarise(crashes = sum(unique_crashes)) %>%
    ggplot(aes(x = HOUR, crashes)) +
        geom_line()+
