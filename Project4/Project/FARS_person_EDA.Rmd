...
involved in one collision, the less rare it is although there seems to have been
at least one collision with almost 100 people involved.
...
this isn't so straightforward though since different months have different number 
of days.
...
incidents where people are working during the day on weekdays.
...
By spliting the seat position into two features, we can see how offten people 
...
child_restraint_pos$SEAT_ROW <- gsub("(Seat).*", "", child_restraint_pos$pos)
...
Accidents with alcohol involvment are much more frequesnt on the weekend 
evenings. They also increase some durring the evenings thoughout the week 
compared to the middle of the day, when there are very few fatal accidents
with alcohol involvment. Accidents with drug or drug and alcohol involvment
also are the highest durring the weekend evening, although not nearly to the
...
makes sense because it is recomended not to put them in the front row 
(especially with airbags) and most cars have two rows. The second most common is
the thrid row (vans and SUVs). We can also see that most frequently the
...
it appears to actually have the lowest survivor rate od any second or third 
row seat.
...
To start trying to wrap my head arround this data set, I started by exploring
a few of the more prominant features individually, but with how much 
...
different situatons and unders what circumstances do accidents with fatalities 
happen.

There were some relationships that seemed pretty clear, such as using child
restraints saving childrens lives and drugs and alcohol being
a factor more frequently on weekend evenings. However with there being 
so many conflaiting factors to consider a lot of the relationships were less 
clear. 
...
and vehilce make/model from the vehicle file and combining it with our data 
